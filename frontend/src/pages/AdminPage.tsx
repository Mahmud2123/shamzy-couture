...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "orders" | "users" | "measurements" | "products" | "audit"
  >("overview");
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          <div
            key={card.label}
            className={`bg-white rounded-xl p-5 shadow-sm ${(card as any).span ? "col-span-2 lg:col-span-1" : ""}`}
          >
            <div
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm">
        <h2 className="font-bold text-stone-800 mb-4 flex items-center gap-2">
          <FiTrendingUp /> Orders by Status
        </h2>
        <div className="flex flex-wrap gap-3">
          {Object.entries(ordersByStatus).map(([status, count]) => (
            <div
              key={status}
              className={`px-4 py-2 rounded-lg text-sm font-semibold ${STATUS_COLORS[status] || "bg-stone-100 text-stone-700"}`}
            >
...
// ── Orders Tab ────────────────────────────────────────────────────────────────
function OrdersTab() {
  const [data, setData] = useState<{ orders: Order[]; total: number } | null>(
    null,
  );
  const [loading, setLoading] = useState(true);
  ...
  const handleUpdate = async () => {
    ...
    setUpdating(true);
    try {
      const updated = await adminService.updateOrder(selectedOrder.id, {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="flex-1 min-w-48 border border-stone-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
        />
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="border border-stone-300 rounded-lg px-4 py-2 text-sm focus:outline-none"
        >
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="h-16 bg-stone-200 rounded-xl animate-pulse"
            />
          ))}
        </div>
      ) : data?.orders.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl text-stone-500">
          No orders found
        </div>
      ) : (
            ...
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 font-semibold text-stone-600 text-xs uppercase"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {data?.orders.map((order) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[order.status]}`}
                    >
                      {order.status.replace("_", " ")}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="bg-stone-900 text-white px-3 py-1 rounded-lg text-xs hover:bg-stone-700"
                    >
                      Manage
                    </button>
                  </td>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
              <div>
                <p className="text-stone-400 text-xs">Customer</p>
            ...
            <div className="mb-4">
              ...
              {selectedOrder.items?.map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between py-1 border-b border-stone-100 text-sm"
                >
                  <span>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {selectedOrder.statusLogs.map((log) => (
                      ...
                      >
                        <span
                          className={`px-2 py-0.5 rounded-full ${STATUS_COLORS[log.status]}`}
                        >
                          {log.status.replace("_", " ")}
                        </span>
                        {log.note && (
                          <span className="text-stone-400">— {log.note}</span>
                        )}
            ...
            <div className="bg-stone-50 rounded-xl p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Fabric sourced"
                    className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm focus:outline-none"
                  />
                </div>
              </div>
              ...
              <button
                ...
                disabled={updating}
                className="w-full bg-stone-900 text-white py-2 rounded-xl font-semibold text-sm hover:bg-stone-700 disabled:opacity-60"
              >
...
// ── Users Tab ─────────────────────────────────────────────────────────────────
function UsersTab() {
  const [data, setData] = useState<{ users: any[]; total: number } | null>(
    null,
  );
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  ...
  return (
    <div className="space-y-4">
      <input
        ...
        className="w-full max-w-md border border-stone-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
      />
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="h-14 bg-stone-200 rounded-xl animate-pulse"
            />
          ))}
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                {["User", "Role", "Orders", "Status", "Joined", "Action"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 font-semibold text-stone-600 text-xs uppercase"
            ...
            <tbody>
              {data?.users.map((user) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${user.role === "ADMIN" ? "bg-amber-100 text-amber-700" : "bg-stone-100 text-stone-600"}`}
                    >
                      {user.role}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${user.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
                    >
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 bg-stone-50 rounded-lg p-3">
                {[
                  ...
                  ["Measurements", selectedUser._count?.measurements],
                  ["Design Reqs", selectedUser._count?.designRequests],
                ].map(([l, v]) => (
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="w-full bg-stone-900 text-white py-2.5 rounded-xl font-semibold hover:bg-stone-700 disabled:opacity-60"
              >
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-stone-900 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-stone-700"
        >
          + New Product
        </button>
      </div>
              ...
              {products.map((p) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 flex gap-2">
                    <button
                      onClick={() => openEdit(p)}
                      className="text-xs bg-stone-100 px-3 py-1 rounded-lg hover:bg-stone-200"
                    >
                      Edit
                    </button>
                    <button
...
function MeasurementsTab() {
  ...
  const load = () => {
    setLoading(true);
    adminService
      .getMeasurements({ productType: typeFilter || undefined })
      .then(setData)
      .catch(() => toast.error("Failed"))
      .finally(() => setLoading(false));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {["SUIT", "SHIRT", "TROUSER", "JACKET", "CUSTOM"].map((t) => (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                {["Customer", "Label", "Type", "Unit", "Default", "Date"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 font-semibold text-stone-600 text-xs uppercase"
          ...
          <tbody>
            {data?.logs.map((log) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-semibold ${ACTION_COLORS[log.action] || "bg-stone-100 text-stone-600"}`}
                  >
